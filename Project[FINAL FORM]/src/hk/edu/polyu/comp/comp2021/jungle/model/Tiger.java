...
public class Tiger extends Beast {
    public Tiger(boolean available, int x, int y, int priority, int ownership) {
    ...
    public boolean isValid(Terrain terrain, int fromx, int fromy, int tox, int toy) {
        ...
        /*
       FROM aroundriver TO river isValid conditions for Tiger:
        1. Aroundriver tiger cannot move to river if there is a beast in Leap Path
        */
            ...
            if (toy == fromy + 4) {
                ...
                for(int i = (fromy+1); i <= (fromy+3); i++) {   // Leap Path
                    if (terrain.positions[fromx][i].getBeast() != null) {
                        ...
                        System.out.println("[TIGER][From aroundriver to river][UPWARD LEAP][ERROR] Cannot leap if there is a beast");
                        return false;
                    }
                }
            }
            if (toy == fromy + 1) {
                System.out.println("[TIGER][From aroundriver to river][ERROR] Tiger must perform a leap accross river. Please adjust input.");
                return false;
            }
        }
        ...
        if (fromy == 6 && fromx != 0 && fromx != 6 && fromx != 3) {
            if (toy == fromy - 4) {
                ...
                for(int i = (fromy-1); i >= (fromy-3); i--) {   // Leap Path
                    if (terrain.positions[fromx][i].getBeast() != null) {
                        System.out.println("[TIGER][From aroundriver to river][DOWNWARD LEAP][ERROR] Cannot leap if there is a beast");
                        return false;
                    }
                }
            }
        ...
        if ((fromx == 0 || fromx == 3 || fromx == 6) && (fromy == 3 || fromy == 4 || fromy == 5)) {
            if (tox == fromx + 3) {
                ...
                for(int i = (fromx+1); i <= fromx + 2; i++){
                    if (terrain.positions[i][fromy].getBeast() != null) {
                        System.out.println("[TIGER][From aroundriver to river][RIGHTWARD LEAP][ERROR] Cannot leap if there is a beast");
                        return false;
                    }
                }
            }
            if(tox == fromx - 3) {
                ...
                for(int i = (fromx-1); i >= fromx - 2; i--){
                    if (terrain.positions[i][fromy].getBeast() != null) {
                        System.out.println("[TIGER][From aroundriver to river][LEFTWARD LEAP][ERROR] Cannot leap if there is a beast");
                        return false;
                    }
                }
            }
        ...
        if (upwardleapdetected || downwardleapdetected || rightwardleapdetected || leftwardleapdetected){
            System.out.println("[TIGER][SPECIAL SKILL] Leap completed.");
            return true;
        }
        // [General Test] can only move 1 spot version y
        if((toy != (fromy + 1)  && tox == fromx)    &&  (toy != (fromy - 1)  && tox == fromx)){

            System.out.println("[TIGER][General Test][Error] to-y is wrong");
            return false;
        }
        ...
        if((tox != (fromx + 1) && toy == fromy)     &&  (tox != (fromx - 1) && toy == fromy)){
            System.out.println("[TIGER][General Test][Error] to-x is wrong");
            return false;
        }
